...
readMATmod <- function(file){
  ...
  # XXX mj XXX
  model@met_single <- rep(NA, model@met_num)
  model@met_de <- rep(NA, model@met_num)
  model@react_single <- rep(NA, model@react_num)
  model@react_de <- rep(NA, model@react_num)
  model@rxnGeneMat <- Matrix::Matrix(FALSE,
                                 nrow = model@react_num,
                                 ncol = length(model@allGenes),
                                 sparse = TRUE)
